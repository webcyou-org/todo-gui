...
fn cargo_link_qt(env: &str, target_family: &str) {
    ...
    if target_os == "macos" {
        // Qt is installed as .framework bundles on macOS (Homebrew)
        ...
        println!("cargo:rustc-link-lib=framework=QtOpenGL");
        // c++ stdlib comes from libc++ on macOS
        ...
        if target_family == "unix" && target_os == "linux" {
